...
if [ -f "$TEMPLATE_FILE" ]; then
  ...
  envsubst '$FIREBASE_API_KEY $FIREBASE_AUTH_DOMAIN $FIREBASE_PROJECT_ID $FIREBASE_STORAGE_BUCKET $FIREBASE_MESSAGING_SENDER_ID $FIREBASE_APP_ID $SINGLE_USER_EMAIL $SINGLE_USER_PASSWORD' < "$TEMPLATE_FILE" > "$TARGET_FILE"
